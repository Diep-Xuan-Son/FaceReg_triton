...
batch_size = 10
torch_input = np.random.rand(batch_size, 3, 640, 640).astype(np.float32)
# torch_out = torch_model(torch_input)

ort_session = onnxruntime.InferenceSession("model1.onnx", providers=["CPUExecutionProvider"])
def to_numpy(tensor):
...
ort_inputs = {ort_session.get_inputs()[0].name: torch_input}
